...
- Verification (this run):
  ...
  - `npm test` (jest): PASS — 15 suites, 97 tests (decision engine, safety gate + crisis matcher,
    ...
    integration, day-map projection + deterministic narration, **at-rest encryption: secret-box +
    EncryptedStore migration + EncryptedKV**).
  - `npm run lint` (expo lint): PASS.
  - `npx expo export -p android`: PASS — bundles for Android (1403 modules) with SyncingStore + F-013 +
    F-002 + at-rest encryption.
...
- Independent security/privacy/safety review (2026-09-09): no P0. Safety/consent/lock-screen/RLS
  ...
  - **P2 (at-rest encryption / MASVS-STORAGE-1) — RESOLVED (2026-09-11):** the local document is now
    authenticated-encrypted (tweetnacl secretbox) behind the `Store` port via `EncryptedStore`, with
    the 32-byte key held in the OS keystore (expo-secure-store, device-only) and generated by
    expo-crypto's secure RNG; a one-time migration re-encrypts any legacy plaintext document and
    deletes the plaintext. The Supabase session (SEC P2-1) is likewise encrypted at rest via
    `EncryptedKV` as the auth storage adapter. The old plaintext store was removed. RESIDUAL (device-
    only, TASK-370): verify Keychain/Keystore behavior on real iOS/Android hardware — the crypto +
    migration + adapter logic is unit-tested in Node, but the native keystore path needs a device.
- **Audit #2 (Stage 13, 2026-09-11): CONDITIONAL → resolved-in-scope.** Two independent reviewers
  ...
  - SEC P2-1 (Supabase session token secure storage) — RESOLVED (2026-09-11): session encrypted at
    rest via `EncryptedKV`. SEC P2-3 (app data at rest) — RESOLVED (see above).
  - **Still tracked as PRE-CLOUD-ACTIVATION / PRE-BETA blockers (latent — no auth/session wired yet;
    see ADR-013):** SEC P1-1 full server-side cloud delete (Edge Function); SEC P2-2 explicit "cloud
    backup" consent + "what-leaves-the-device" disclosure + field encryption before any auto-sync of
    special-category data; SEC P3-1 column-scoped `outcomes` SELECT (drop free_note from the client
    read channel; needs explicit-column fetch); SEC P3-2 consent-withdrawal breaks the push pipeline
